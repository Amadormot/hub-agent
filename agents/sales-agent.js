...
async function scrapeProductData(url, platformId) {
    ...
    try {
        ...
        // Validação de Nome Falso/Genérico
        if (title && (title.length < 15 || title.toLowerCase() === platformId.replace('_', ' '))) {
            console.log(`⚠️ Título considerado genérico ou curto demais: ${title}`);
            title = null;
        }
    ...
    for (const category of shuffledCategories) {
        ...
        for (const keyword of shuffledKeywords) {
            ...
            for (const p of trendingProducts) {
                ...
                // EXTRAÇÃO DE DADOS REAIS (Preço + Título)
                ...
                // POLÍTICA DE NOMES REAIS & PREÇO
                if (!realPrice || !realTitle) {
                    console.log(`🚫 REJEITADO: Dados reais (nome/preço) incompletos para ${keyword}.`);
                    continue;
                }
                ...
                const productRecord = {
                    name: realTitle, // 100% nome da loja
                    price: realPrice,
                    image: image,
