export default function Logo({ size = 40, className = "" }) {
    return (
        <div className={`relative flex items-center justify-center ${className}`} style={{ width: size, height: size }}>
            <img
                src="/logo.jpg"
                alt="Jornada Biker Logo"
                className="w-full h-full object-contain drop-shadow-md rounded-lg"
                onError={(e) => {
                    e.target.style.display = 'none';
                }}
            />
            <svg viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full drop-shadow-xl absolute top-0 left-0 -z-10 opacity-0">
                {/* Fallback SVG logic if needed, but keeping it hidden for now as we trust logo.jpg */}
